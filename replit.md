...
- **Postal Code and Country Fields**: Added postal code and country selection across all user-facing forms. Created shared/countries.ts with 195+ countries (South Africa listed first as default). Updated candidateProfiles database table with postalCode (nullable text) and country (required text, defaults to 'South Africa'). Updated cvPersonalInfoSchema to include province, postalCode, city, and country fields. Integrated fields into individual onboarding form (positioned: City/Province row, Postal Code/Country row) and CV Builder PersonalInfoStep with same positioning. Country dropdown defaults to South Africa, postal code is optional. Province field is conditional: shows SA province dropdown when South Africa is selected, switches to text input for other countries. Database schema migration successful, forms tested and working.
