...
- **Backend**: Built with Express.js and TypeScript.
    - **API Endpoints**: Manages subscriptions, job postings, CVs, roles/screening, ATS, organization settings, competency testing, interview scheduling (OAuth flows for Google/Microsoft/Zoom, availability checking, interview booking/rescheduling/canceling), corporate clients management (client CRUD, contacts with POPIA consent, fee agreements, client-specific jobs and analytics with organization-scoped sharing), contact form inquiries, interview coach interactions, and billing management including specific endpoints for job status management and conditional validation. CV endpoints include authorization checks.
    - **Billing System**: Three role-specific dashboards (Individual at `/dashboard/individual/billing`, Recruiter/Corporate at `/dashboard/recruiter/billing`, Admin at `/admin/billing`) showing current plan, usage quotas, feature access, and upgrade paths. Automated monthly billing reset cron job runs daily at midnight to process expired billing periods, reset usage counters, and handle scheduled cancellations. Admin can manually trigger reset via `/api/admin/billing/reset-usage` endpoint for testing/maintenance. Usage reset strategy deletes old usage records when billing period ends. **Admin Feature & Plan Management** (November 2025): Complete CRUD interfaces at `/admin/features` and `/admin/plans` for managing platform features (16 total with TOGGLE/QUOTA/METERED types) and subscription plans (18 total across 3 products × 3 tiers × 2 intervals). Features include searchable tables, inline editing, cascade deletion protection, feature entitlement configuration dialog for assigning features to plans with monthly caps, and comprehensive validation preventing primary key changes.
...
- **Data Storage**: Utilizes PostgreSQL (Neon) with Drizzle ORM and the pgvector extension, employing UUID primary keys.
    ...
    - **Job SEO Metadata** (November 2025): `jobs` table includes `seo` JSONB field storing AI-generated and manually edited SEO metadata with structure: `{ slug, titleTag (≤60 chars), metaDescription (≤155 chars), ogTitle (≤70), ogDescription (≤200), twitterTitle (≤70), twitterDescription (≤200), imageAlt, keywords[], hashtags[], internalLinks[], faq[], jsonld (schema.org JobPosting structured data), version }`. SEO generation uses OpenAI GPT-4o with South African context, ZAR currency formatting, and character limit enforcement. API endpoints (`/api/jobs/:jobId/seo/generate`, `/api/jobs/:jobId/seo/save`) enforce recruiter role and job ownership authorization. Meta tags and JSON-LD rendered on job detail pages via useEffect hook.
