...
- **Frontend**: React with TypeScript (Vite), Wouter for routing, and TanStack React Query for state management.
    ...
    - **Competency Test Agent** (November 2025): AI-powered assessment platform with unique `TEST-XXXXXX` reference numbers, supporting three creation methods (AI-generated from job description, manual authoring, pre-built template library). Features standalone candidate portal, multi-format questions (MCQ, SJT, Likert, work samples), South African compliance (POPIA notices, EEA/HPCSA alignment), basic anti-cheating (IP logging, randomization, fullscreen monitoring), and ATS integration where test scores automatically update candidate pipeline status.
