...
- **Frontend**: Developed with React and TypeScript (Vite), using Wouter for routing and TanStack React Query for state management.
    ...
    - **Individuals Portal**: Supports multiple CV management (with AI-powered circular cropping for photos and PDF previews), profile management, competency test access, and an AI Interview Coach.
        - **Job Searches Collapsible Tree** (November 2025): Reorganized Job Searches section into a collapsible sidebar tree structure with three sub-sections:
            ...
            - **Manual Job Search** (`/dashboard/individual/jobs/manual`): Traditional browse and filter interface with search bar, location/industry/type filters, and job cards displaying transparent salary ranges with WhatsApp application
