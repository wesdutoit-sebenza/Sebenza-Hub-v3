...
- **Frontend**: React with TypeScript (Vite), Wouter for routing, and TanStack React Query for state management.
    - **Recruiters Portal**: Job posting forms with WhatsApp integration, an AI Job Description Generator, and a status-based workflow (Draft, Live, Paused, Closed, Filled) with conditional validation. Unique `JOB-XXXXXX` reference numbers are assigned automatically.
        - **Sticky Form Navigation** (November 2025): Interactive sidebar navigation for job posting form with 11 sections. Uses Intersection Observer for automatic active section tracking. Smooth scroll behavior with offset adjustment for sticky header. Responsive design: hidden on mobile, sticky sidebar on desktop (lg+). Component: `JobFormNavigation.tsx`.
        - **Enhanced Skills Structure** (November 2025): Required Skills transformed from simple string arrays to rich skill objects with level (Basic/Intermediate/Expert) and priority (Must-Have/Nice-to-Have) attributes. Each skill displays inline Select dropdowns for level and priority configuration. Default values: level="Intermediate", priority="Must-Have". Includes backward compatibility migration layer that automatically converts legacy string[] skills to new object[] format server-side.
        ...
        - **AI Company Description Assistant** (November 2025): AI-powered company description generation from company websites. Scrapes company home page + about page, then uses GPT-4o to generate 5-10 line company descriptions with tone selection (Professional, Formal, Approachable, Concise, Detailed, Auto-Select). Integrated into job posting form with AI Assistant button above Company Description field. Endpoints: POST /api/jobs/scrape-website (website scraping with SSRF protection) and POST /api/jobs/generate-company-description (AI generation).
